...
function sendONZ (account, i, done) {
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomONZ / node.normalizer;
		}
		done();
	});
}

function sendONZFromMultisigAccount (password, amount, recipient, done) {
	node.put('/api/transactions/', {
		secret: password,
		amount: amount,
...
before(function (done) {
	sendONZ(multisigAccount, null, done);
});
	...
	describe('valid multisigs transactions with different min and keysgroup values', function () {
		...
		it('using valid params should be ok for min 1 and keysgroup array 1', function (done) {
			var params = {
				...
				lifetime: 11,
				min: 1,
				keysgroup: ['+' + node.onz.crypto.getKeys(node.randomPassword()).publicKey]
			};
		...
		it('using valid params should be ok when min 5 and keysgroup array 5', function (done) {
			var params = {
				...
				lifetime: 11,
				min: 5,
				keysgroup: Array.apply(null, Array(5)).map(function () { return '+' + node.onz.crypto.getKeys(node.randomPassword()).publicKey; })
			};
...
describe('PUT /api/transactions', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendONZFromMultisigAccount(multisigAccount.password, 100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
