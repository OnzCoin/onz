...
before(function (done) {
	setTimeout(function () {
		sendONZ(account, 100 * node.normalizer, done);
	}, 2000);
});

before(function (done) {
	setTimeout(function () {
		sendONZ(account2, 20 * node.normalizer, done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 20 ONZ
		sendONZ(account2, 100 * node.normalizer, done);
	}, 2000);
});
...
describe('GET /api/transactions', function () {
	...
	it('using minAmount with and:maxAmount ordered by amount and limited should be ok', function (done) {
		...
		var offset = 0;
		var orderBy = 'amount:asc';
		var minAmount = 20 * node.normalizer; // 20 ONZ
		var maxAmount = 100 * node.normalizer; // 100 ONZ
