...
/**
 * Main entry point.
 * Loads the onz modules, the onz api and run the express server as Domain master.
...
/**
 * The config object to handle onz modules and onz api.
	...
	async.auto({
		...
		listen: ['ready', function (scope, cb) {
			scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
				scope.logger.info('Onz started: ' + scope.config.address + ':' + scope.config.port);

				if (!err) {
					if (scope.config.ssl.enabled) {
						scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
							scope.logger.info('Onz https started: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
		...
		if (err) {
			 ...
			 * @property {Object} ed - Crypto functions from onz node-sodium.
			 ...
			 * @property {string} public - Path to onz public folder.
